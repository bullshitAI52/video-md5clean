...
- 🔁 **失败不能重试**：处理失败只能重新上传，无一键重试
- 💣 **服务重启丢排队**：重启时正在排队的任务丢失（已完成的历史不受影响，会自动恢复）
- 👤 **单账号**：只有一套 nginx basic auth，无用户系统/权限分级
- 🎞 **不能预览**：处理前不能在线预览视频
- 🐢 **单线程处理**：1 核机器设计，任务逐个排队，大文件深度清洗较慢
- ⏰ **文件只保留 24h**：非永久存储，过期自动清除（隐私优先的取舍）
- 🌐 **无独立 HTTPS**：依赖 nginx 反代，Flask 只监听 127.0.0.1
- 🛡 **无速率限制**：接口没有限流，被恶意刷会占满队列
